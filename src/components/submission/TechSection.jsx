    ...
    return (
        <section className="bg-mars-dark text-white rounded-[3rem] p-8 md:p-12 border border-white/10 shadow-2xl relative overflow-hidden">
            ...
            <div className="flex items-center gap-4 border-b border-white/10 pb-6 mb-10 relative z-10">
                ...
                <div>
                    <h2 className="text-lg font-black uppercase tracking-widest leading-tight"><span className="opacity-50 mr-2">03.</span>{t('submission.sections.tech.title')}</h2>
            ...
            <div className="grid md:grid-cols-2 gap-8 relative z-10">
                ...
                <div className="space-y-4">
                    ...
                    <div onClick={() => setValue('tech.hasSubs', !hasSubs)} className={`flex items-center gap-4 p-5 rounded-2xl border cursor-pointer transition-all group ${hasSubs ? 'bg-primary/10 border-primary' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}>
                        ...
                        <span className={`text-[10px] font-black uppercase transition-colors ${hasSubs ? 'text-white' : 'text-slate-400 group-hover:text-white'}`}>{t('submission.sections.tech.subtitles_label')}</span>
                    </div>
                </div>

                {/* TOOLS & PROCESS */}
                <div className="space-y-2">
                    <div className="flex justify-between"><Label className="text-slate-400">{t('submission.sections.tech.tools')}</Label><span className={`text-[8px] font-bold transition-colors ${toolsUsed.length >= 500 ? 'text-red-500' : 'text-slate-500'}`}>{toolsUsed.length}/500</span></div>
                    <TextArea rows={4} placeholder={t('submission.placeholders.tools')} {...register("tech.toolsUsed", { required: "Requis", maxLength: 500 })} className={`bg-white/5 text-white placeholder:text-slate-500 focus:bg-white/10 focus:border-primary ${errors.tech?.toolsUsed || toolsUsed.length >= 500 ? 'border-red-500' : 'border-white/10'}`} />
                    {errors.tech?.toolsUsed && <ErrorMessage message={errors.tech.toolsUsed.message} />}
                </div>
                <div className="space-y-2">
                    <div className="flex justify-between"><Label className="text-slate-400">{t('submission.sections.tech.process')}</Label><span className={`text-[8px] font-bold transition-colors ${creativeProcess.length >= 500 ? 'text-red-500' : 'text-slate-500'}`}>{creativeProcess.length}/500</span></div>
                    <TextArea rows={4} placeholder={t('submission.placeholders.process')} {...register("tech.creativeProcess", { required: "Requis", maxLength: 500 })} className={`bg-white/5 text-white placeholder:text-slate-500 focus:bg-white/10 focus:border-primary ${errors.tech?.creativeProcess || creativeProcess.length >= 500 ? 'border-red-500' : 'border-white/10'}`} />
                ...
                <div className="md:col-span-2 space-y-2">
                    ...
                    <div onClick={() => fileInputRef.current?.click()} className={`w-full h-48 border-2 border-dashed rounded-3xl bg-white/5 flex flex-col items-center justify-center group hover:border-primary hover:bg-primary/5 transition-all cursor-pointer ${errors.tech?.cover ? 'border-red-500' : 'border-white/10'}`}>
                        {coverFile && coverFile.length > 0 ? (
                            ...
                            <><ImageIcon className="w-10 h-10 text-white/10 group-hover:text-primary transition-all mb-4" /><span className="text-[10px] font-black text-slate-500 group-hover:text-white uppercase tracking-widest">{t('submission.sections.tech.cover_drag')}</span><span className="text-[8px] font-bold text-slate-600 mt-1">JPG / PNG (Max 2Mo)</span></>
                        )}
                    </div>
