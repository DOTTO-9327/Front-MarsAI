    ...
    return (
        <section className="bg-mars-dark text-white rounded-[3rem] p-8 md:p-12 border border-white/10 shadow-2xl relative overflow-hidden">
            ...
            <div className="grid md:grid-cols-2 gap-8 relative z-10">
                ...
                {/* --- CLASSIFICATION (Boutons Custom) --- */}
                {/* Mettre en place la validation des button avec RHF */}
                ...
                <div className="space-y-2">
                    <div className="flex justify-between">
                        ...
                        <span className={`text-[8px] font-bold transition-colors ${toolsUsed.length >= 500 ? 'text-red-500' : 'text-slate-500'}`}>{toolsUsed.length}/500</span>
                    </div>
                    <TextArea
                        rows={4}
                        placeholder={t('submission.placeholders.tools')}
                        {...register("tech.toolsUsed", {
                            required: "La liste des outils est requise",
                            maxLength: { value: 500, message: "Trop long (max 500)" }
                        })}
                        className={`bg-white/5 text-white placeholder:text-slate-500 focus:bg-white/10 focus:border-primary ${errors.tech?.toolsUsed || toolsUsed.length >= 500 ? 'border-red-500' : 'border-white/10'}`}
                    />
                ...
                <div className="space-y-2">
                    <div className="flex justify-between">
                        ...
                        <span className={`text-[8px] font-bold transition-colors ${creativeProcess.length >= 500 ? 'text-red-500' : 'text-slate-500'}`}>{creativeProcess.length}/500</span>
                    </div>
                    <TextArea
                        rows={4}
                        placeholder={t('submission.placeholders.process')}
                        {...register("tech.creativeProcess", {
                            required: "Le processus créatif est requis",
                            maxLength: { value: 500, message: "Trop long (max 500)" }
                        })}
                        className={`bg-white/5 text-white placeholder:text-slate-500 focus:bg-white/10 focus:border-primary ${errors.tech?.creativeProcess || creativeProcess.length >= 500 ? 'border-red-500' : 'border-white/10'}`}
                    />
                ...
                <div className="md:col-span-2 space-y-2">
                    ...
                    {/* Zone de drag & drop (Pour l'instant visuel uniquement, nécessite un input type="file" caché pour validation RHF) */}
                    ...
                    {/* Placeholder pour future erreur d'upload */}
                </div>
            </div>
        </section>
    );
